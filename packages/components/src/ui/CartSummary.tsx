...
export function CartSummary({ cart, layout }: CartSummaryProps) {
  ...
  return (
    <div aria-labelledby="cart-summary" className={className}>
      ...
      <CartGiftCard giftCardCodes={cart?.appliedGiftCards as any} />
...
function CartDiscounts({ discountCodes }: { discountCodes?: CartApiQueryFragment['discountCodes'] }) {
  const codes: string[] =
    discountCodes?.filter((discount: any) => discount.applicable)?.map(({ code }: any) => code) || [];
    ...
    <div>
      ...
      {giftCardCodes && giftCardCodes.length > 0 && (
        <dl>
          ...
          {giftCardCodes.map((giftCard: any) => (
