...
import { useAnalytics, useOptimisticCart, type CartViewPayload } from '@shopify/hydrogen';
...
export function HeaderMenu({
  ...
  return (
    <nav className={className} role="navigation">
      {viewport === 'mobile' && (
        <LocalizedLink to="/" onClick={close} className="header-menu-item">
          Home
        </LocalizedLink>
      )}
      {(menu || FALLBACK_HEADER_MENU).items.map(item => {
        ...
        const url =
          ...
          item.url.includes(primaryDomainUrl)
            ...
            : item.url;
        return (
          <LocalizedLink key={item.id} to={url} className="header-menu-item" onClick={close}>
            {item.title}
          </LocalizedLink>
        );
      })}
...
function HeaderCtas({ isLoggedIn, cart }: Pick<HeaderProps, 'isLoggedIn' | 'cart'>) {
  return (
    <nav className="header-ctas" role="navigation">
      ...
      <LocalizedLink to="/account" className="header-menu-item">
